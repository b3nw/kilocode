...
class CommitMessageService {
     ...
     * The VSCode extension automatically detects staged changes first,
     * then falls back to unstaged changes if none exist.
     *
    ...
    suspend fun generateCommitMessage(
        ...
        return try {
            ...
            val rpcResult = executeRPCCommand(proxy, workspacePath)
            ...
            Result.Error(I18n.t("kilocode:commitMessage.generationFailed", mapOf("errorMessage" to (e.message ?: "Unknown error"))))
        }
    }
    ...
    private suspend fun executeRPCCommand(
        ...
        // Execute the command via RPC - pass workspacePath as string like KiloCodeAuthProtocolCommand
        // VSCode function: generateCommitMessageExternalByPath(workspacePath?: string)
        val promise: LazyPromise = proxy.executeContributedCommand(
            CommitMessageConstants.EXTERNAL_COMMAND_ID,
            listOf(workspacePath)
        )
